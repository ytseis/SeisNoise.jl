...
function summary(R::RawData)
    string(typeof(R), " with ", length(R.t), " window", size(R.x, 1) == 1 ? "" : "s")
end
function summary(F::FFTData)
    string(typeof(F), " with ", length(F.t), " fft", size(F.fft, 2) == 1 ? "" : "s")
end
function summary(C::CorrData)
    string(
        typeof(C), " with ", length(C.t), " correlation", size(C.corr, 1) == 1 ? "" : "s"
    )
end
...
function show(io::IO, R::RawData)
    ...
    for f in Field
        if (f in unindexed_fields) == false
            ...
            elseif (
                t <: AbstractFloat || t <: InstrumentPosition || t <: InstrumentResponse
            )
            ...
            elseif f == :t
                if length(R.t) > 0
                    SeisBase.show_str(
                        io,
                        String[timestamp(R.t[1]), " (", string(length(R.t)), " windows)"],
                        w,
                        N,
                    )
                else
                    SeisBase.show_str(
                        io, String[repr("text/plain", targ; context=:compact=>true)], w, N
                    )
                end
            elseif f == :x
        ...
        if (f in unindexed_fields) == false
            ...
            elseif f == :t
                if length(F.t) > 0
                    SeisBase.show_str(
                        io,
                        String[timestamp(F.t[1]), " (", string(length(F.t)), " FFTs)"],
                        w,
                        N,
                    )
                else
        ...
        if (f in unindexed_fields) == false
            ...
            elseif f == :t
                if length(C.t) > 0
                    SeisBase.show_str(
                        io,
                        String[timestamp(C.t[1]), " (", string(length(C.t)), " Corrs)"],
                        w,
                        N,
                    )
                else
