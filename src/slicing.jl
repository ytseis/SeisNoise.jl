...
function slide(
    ...
    elseif cc_step == cc_len # disregard data from edge
        return Array(
            reshape(A[1:(N - N % window_samples)], window_samples, N ÷ window_samples)
        ),
        starts
    else # need overlap between windows
        out = Array{eltype(A),2}(undef, window_samples, length(starts))
        s = convert.(Int, round.((hcat(starts, ends) .- starttime) .* fs .+ 1.0))
...
- `starts::Array`: Array of start times of each window, in Unix time. E.g to convert
        Unix time to date time, use u2d(starts[1]) = 2018-08-12T00:00:00
"""
function slide(C::SeisChannel, cc_len::Real, cc_step::Real)
    slide(C.x, cc_len, cc_step, C.fs, d2u.(start_end(C))...)
end
...
"""
function nearest_start_end(
    S::DateTime, E::DateTime, fs::Float64, cc_len::Real, cc_step::Real
)
    ideal_start = DateTime(Date(S)) # midnight of same day
    starts = Array(ideal_start:Second(cc_step):E)
    ends = starts .+ Second(cc_len) .- Millisecond(convert(Int, 1.0 / fs * 1e3))
    return d2u(starts[findfirst(x -> x >= S, starts)]),
    d2u(ends[findlast(x -> x <= E, ends)])
end

function slide_ind(
    startslice::AbstractFloat, endslice::AbstractFloat, fs::AbstractFloat, t::AbstractArray
)
