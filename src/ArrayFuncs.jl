...
function demean(A::AbstractArray{<:AbstractFloat}; dims=1)
    (U=deepcopy(A); demean!(U; dims=dims); return U)
end
...
function taper!(
    ...
    fs::Real;
    max_percentage::AbstractFloat=0.05,
    max_length::Real=20.0,
)
    Nrows = size(A, 1)
    T = eltype(A)
    wlen = min(
        Int(floor(Nrows * max_percentage)), Int(floor(max_length * fs)), Int(floor(Nrows/2))
    )
...
function taper(
    ...
    (
        U=deepcopy(A);
        taper!(U, fs; max_percentage=max_percentage, max_length=max_length);
        return U
    )
end
function taper!(R::RawData; max_percentage::AbstractFloat=0.05, max_length::Real=20.0)
    taper!(R.x, R.fs; max_percentage=max_percentage, max_length=max_length)
end
function taper(R::RawData; max_percentage::AbstractFloat=0.05, max_length::Real=20.0)
    (
        U=deepcopy(R);
        taper!(U.x, U.fs; max_percentage=max_percentage, max_length=max_length);
        return U
    )
end
function taper!(C::CorrData; max_percentage::AbstractFloat=0.05, max_length::Real=20.0)
    taper!(C.corr, C.fs; max_percentage=max_percentage, max_length=max_length)
end
function taper(C::CorrData; max_percentage::AbstractFloat=0.05, max_length::Real=20.0)
    (
        U=deepcopy(C);
        taper!(U.corr, U.fs; max_percentage=max_percentage, max_length=max_length);
        return U
    )
end
function taper!(N::NodalData; max_percentage::AbstractFloat=0.05, max_length::Real=20.0)
    taper!(N.data, N.fs[1]; max_percentage=max_percentage, max_length=max_length)
end
function taper(N::NodalData; max_percentage::AbstractFloat=0.05, max_length::Real=20.0)
    (
        U=deepcopy(N);
        taper!(U.data, U.fs[1]; max_percentage=max_percentage, max_length=max_length);
        return U
    )
end
...
function hanningwindow(A::AbstractArray, n::Int)
    T = eltype(A)
    win = similar(A, T, n)
    win .= T(pi) .* range(T(0.0); stop=T(n), length=n)
    win .= sin.(win) .^ 2
    return win
end
