...
function smooth!(A::AbstractArray, half_win::Int=3, dims::Int=1)
    ...
    csumsize = tuple(
        collect(size(A)) .+ [i==1 ? 2 * (window_len - 1) + 1 : 0 for i in 1:ndims(A)]...
    )
    ...
    A[:, :] .=
        (
            csum[(window_len + half_win + 1):(end - half_win), :] .-
            csum[(half_win + 1):(end - window_len - half_win), :]
        ) ./ weight
    return nothing
end
function smooth(A::AbstractArray, half_win::Int=3, dims::Int=1)
