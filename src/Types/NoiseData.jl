...
mutable struct RawData <: NoiseData
    ...
    function RawData(S::SeisData, cc_len::Real, cc_step::Real)
        ...
        x, starts = slide(
            @view(S.x[1][startind:endind]), cc_len, cc_step, S.fs[1], startslice, endslice
        )
        return new(
            S[1].id,
            Dates.format(u2d(starts[1]), "Y-mm-dd"),
            S.loc[1],
            S.fs[1],
            S.gain[1],
            1.0 / cc_len,
    ...
    function RawData(C::SeisChannel, cc_len::Real, cc_step::Real)
        ...
        if mod(cc_len * C.fs, 1.0) !== 0.0
            throw(DomainError(cc_len, "cc_len * fs must be an integer for slicing."))
        end

        # check cc_step and sampling rate
        if mod(cc_step * C.fs, 1.0) !== 0.0
            throw(DomainError(cc_step, "cc_step * fs must be an integer for slicing."))
        end
        ...
        x, starts = slide(
            @view(C.x[startind:endind]), cc_len, cc_step, C.fs, startslice, endslice
        )
        return new(
            C.id,
            Dates.format(u2d(starts[1]), "Y-mm-dd"),
...
function FFTData(;
    ...
    notes::Array{String,1}=Array{String,1}(undef, 0),
    t::Array{Float64,1}=Array{Float64,1}(undef, 0),
    fft::AbstractArray{<:Complex{<:AbstractFloat},2}=Array{Complex{Float32},2}(undef, 0, 2),
)
    FFTData(
        name,
        id,
...
function isequal(N::T, U::T) where {T<:NoiseData}
    return minimum(
        [hash(getfield(N, i))==hash(getfield(U, i)) for i in fieldnames(T)]::Array{Bool,1}
    )
end
...
function Adapt.adapt_structure(to, R::RawData)
    RawData(
        R.name,
        R.id,
        R.loc,
...
function Adapt.adapt_structure(to, F::FFTData)
    FFTData(
        F.name,
        F.id,
        F.loc,
...
function Adapt.adapt_structure(to, C::CorrData)
    CorrData(
        C.name,
        C.id,
        C.loc,
