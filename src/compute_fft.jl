...
function process_raw(S::SeisData, fs::Real; ϕshift::Bool=true)
    (U=deepcopy(S); process_raw!(U, fs; ϕshift=ϕshift); return U)
end
...
function process_raw(C::SeisChannel, fs::Real; ϕshift::Bool=true)
    (U=deepcopy(C); process_raw!(U, fs; ϕshift=ϕshift); return U)
end
...
mute(R::RawData, factor::Real=3) = (U=deepcopy(R); mute!(U, factor); return U)
...
function clip(A::AbstractArray, factor::Real; f::Function=std, dims=1)
    (U=deepcopy(A); clip!(U, factor; f=f, dims=dims); return U)
end
function clip!(R::RawData, factor::Real; f::Function=std, dims=1)
    clip!(R.x, factor; f=f, dims=dims)
end
function clip(R::RawData, factor::Real; f::Function=std, dims=1)
    (U=deepcopy(R); clip!(U.x, factor; f=f, dims=dims); return U)
end
clamp!(R::RawData, val::Real) = clamp!(R.x, -abs(val), abs(val))
function clamp!(R::RawData, lo::Real, hi::Real)
    if lo < hi
        clamp!(R.x, lo, hi)
    else
        throw(ArgumentError("lo value $lo must be less than hi value $hi."))
    end
end
...
"""
function remove_amp!(R::RawData; max_std::Real=10.0)
...
function remove_amp(R::RawData; max_std::Real=10.0)
    (U=deepcopy(R); remove_amp!(U; max_std=max_std); return U)
end
